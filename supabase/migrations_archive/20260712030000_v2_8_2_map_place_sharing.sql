...
-- Version: 20260712030000
-- Name: v2_8_2_map_place_sharing
-- Bu dosyanın version/name değeri remote schema_migrations kaydıyla eşleşmelidir.

-- Statement 1/3
-- v2.8.2 — Create/reuse a Places row only when a map result is actually
-- sent through the in-app sharing flow. Country eligibility is intentionally
...
begin
  ...
  if p_place_id is not null and p_place_id > 0 then
    select place_row."PlaceId", place_row."PublicId"
      into v_place_id, v_public_id
    ...
    if v_place_id is not null then
      return query select v_place_id, v_public_id;
      return;
    end if;
  end if;
